...
tags:
  - tech_share
image: /img/ccBlog/tech_share/online_ddl_support.png
...
由于各数据源对同步数据的消费并不相同，如消息队列只需要解析 Online DDL 后的表结构即可，无需订阅临时表，因此我们需要根据对端数据源的消费模式做出不同的处理。
  ...
  ![](../assets/blog/tech_share/0002/1.png)
...
下图演示了CloudCanal对DDL语句的一些处理:
  ...
  ![](../assets/blog/tech_share/0002/2.png)
...
- 增量任务中，功能列表 -> 参数修改 -> 源数据源配置 -> 参数 **extraDDL** 设置为 **GHOST**
  ![](../assets/blog/tech_share/0002/3.png)
...
- 首先使用数据生成工具实时随机生成数据，增删改比例为（4：4：3）
  ![](../assets/blog/tech_share/0002/4.png)
...
- 更改后的源对端 **表结构一致**
  ![](../assets/blog/tech_share/0002/5.png)
  ![](../assets/blog/tech_share/0002/6.png)

- 源对端进行数据校验 **数据一致**
  ![](../assets/blog/tech_share/0002/7.png)
