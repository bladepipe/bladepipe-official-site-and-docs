...
tags:
  - data_sync_sample
image: /img/ccBlog/data_sync_sample/mysql_mysql_sync.png 
...
- 启动源端造增量数据程序，IUD 比例 30:50:20
  ...
  ![开始造数据.png](../assets/blog/data_sync_sample/1_mysql_mysql_sync/开始造数据.png)

### 创建任务

-  选择源数据库和目标数据库，做好库映射。操作完毕点击下一步。

   ![创建任务第一步.png](../assets/blog/data_sync_sample/1_mysql_mysql_sync/创建任务第一步.png)

-  选择数据同步，并默认勾选数据初始化、DDL 同步，此处暂不勾选做一次性数据校验。在任务增量追上后，我们停掉造数据程序，创建逻辑一致的校验任务，那么结果更加清晰。

   ![截屏2021-07-31 下午2.27.52.png](../assets/blog/data_sync_sample/1_mysql_mysql_sync/截屏2021-07-31-下午2.27.52.png)

-  选择表，并做好表映射，以及去除不想迁移同步的表。操作完毕点击下一步。

   ![数据同步表选择.png](../assets/blog/data_sync_sample/1_mysql_mysql_sync/数据同步表选择.png)

-  选择列，并做好列映射、裁剪掉某些不需要表的列。

   ![列选择.png](../assets/blog/data_sync_sample/1_mysql_mysql_sync/列选择.png)

-  批量设置唯一键为主键。

   ![截屏2021-07-31 下午1.54.01.png](../assets/blog/data_sync_sample/1_mysql_mysql_sync/截屏2021-07-31-下午1.54.01.png)

-  设置数据过滤，如图中 id < 3000。操作完毕点击下一步。

   ![截屏2021-07-31 下午1.53.11.png](../assets/blog/data_sync_sample/1_mysql_mysql_sync/截屏2021-07-31-下午1.53.11.png)

-  确认创建任务。确认完毕点击创建任务。

   ![数据同步总结.png](../assets/blog/data_sync_sample/1_mysql_mysql_sync/数据同步总结.png)
...
- 数据初始化完毕，开始进行增量同步。增量同步会自动从数据初始化位点拉取 MySQL binlog 进行回放，同样会执行映射、裁剪、以及数据过滤等操作。
  ...
  ![任务追平前夕.png](../assets/blog/data_sync_sample/1_mysql_mysql_sync/任务追平前夕.png)
...
- 数据差异数据目前需要到任务日志目录获取，暂时未提供日志下载能力。
  ...
  ![校验结果.png](../assets/blog/data_sync_sample/1_mysql_mysql_sync/校验结果.png)
