...
tags:
  - data_sync_sample
image: https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/ccBlog/data_sync_sample/tdengine_mysql_sync.png 
...
![](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/data_sync_sample/tdengine_mysql_sync/tdengine_mysql_1.png)
...
3. 选择 **数据同步** 并勾选 **全量初始化**。
   ...
   :::
   ![](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/data_sync_sample/tdengine_mysql_sync/tdengine_mysql_3.png)
4. 选择需要同步的表。
5. 选择表对应的列。
...
6. 点击 **确认创建**。
   ...
   TDengine 源端的任务创建会有以下几个步骤：
    ...
    - 完成任务创建
   :::
7. 进入任务详情页，点击右上角 **功能列表** > [**修改参数**](https://www.clougence.com/cc-doc/operation/job_manage/job_op/job_params)，修改以下任务参数：   
  ...
  ![](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/data_sync_sample/tdengine_mysql_sync/tdengine_mysql_7.png)
8. 启动任务，CloudCanal 会自动进行任务流转，其中的步骤包括：
    - **全量数据迁移**: 已存在的存量数据将会完整迁移到对端。
    - **增量数据同步**: 增量数据将会持续地同步到对端数据库，并且保持实时（秒级别延迟）。
  
  ![](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/data_sync_sample/tdengine_mysql_sync/tdengine_mysql_8.png)
