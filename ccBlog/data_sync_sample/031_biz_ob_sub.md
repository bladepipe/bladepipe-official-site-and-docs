...
tags:
  - data_sync_sample
image: https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/ccBlog/data_sync_sample/biz_ob_sub.png 
...
- **额外参数**：
  ...
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/1.png)
...
- 源端勾选高级配置可以设置OceanBase的租户，默认为sys。CloudCanal的一个任务只支持订阅属于一个租户下的表。此处请设置需要订阅的表所属的租户。
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/2.png)

- **第二步 选择任务类型和规格**，一般需要同时进行全量初始化和长周期增量同步可以按照如图的配置。
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/3.png)

- **第三步 表和Action过滤**，在第三步用户可以勾选自己需要订阅的表以及事件类型
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/4.png)
...
- **第四步 数据处理**：在这一步用户可以进行列裁剪和映射；如果有自定义实时处理数据的需求，在这一步也可以上传自定义代码进行自定义处理。自定义代码处理可以参考[样例工程](https://gitee.com/clougence/cloudcanal-data-process) 。
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/5.png)

- **第五步 确认配置**：这一步用户可以确认任务的完整配置，确认无误后点击创建任务即可完成任务创建的流程。
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/6.png)

### 任务运行
- 任务创建完毕后会自动开始表结构迁移、全量迁移和增量同步
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/7.png)
...
- 支持填写集群模式的host，原有OceanBase数据源的网络地址需要替换成新的格式，即ip:sql_port，或者直接填写obproxy的ip和port。点击 **数据源管理->更多 修改内网/外网地址 **修改**。**
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/8.png)
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/9.png)

- 支持了独立的数据源额外配置参数，点击 **数据源管理->更多->查看配置** 修改。补充obLogProxyHost、rpcPortList等信息
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/10.png)
...
- 创建相似任务，**不勾选自动启动，不勾选全量初始化**，然后点击下一步直到完成相似任务的创建
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/11.png)
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/12.png)

- 停止老的增量任务，并且记录其位点时间戳T1
  ![image.png](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/blog/wufenzhong/0031/13.png)
