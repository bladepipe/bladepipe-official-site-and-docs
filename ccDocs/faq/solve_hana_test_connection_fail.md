...
新增 Sap Hana 数据源时，测试连接出现相关报错, 如下图:
  ![oom](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/docs/faq/hana_connection_fail1.png)
...
1. Sap Hana 执行 `SELECT DATABASE_NAME FROM M_DATABASE;` 查询 **数据库名**。
  ![oom](https://cloudcanal-blog-img.oss-cn-hangzhou.aliyuncs.com/docs/faq/hana_select_db.png)
