...
- 在 **所有节点** 上安装如下基础软件
<Tabs groupId="opsystem">
  <TabItem value="Docker" label="Docker 安装" default>
  ...
  <TabItem value="Kubernetes" label="Kubernetes 工具安装">
  - **CentOS / RHEL**
    :::info
    如果是 arm 架构 CPU，请将以下脚本中的 baseurl 值替换为 https://mirrors.aliyun.com/kubernetes/yum/repos/kubernetes-el7-aarch64
    :::
...
1. 修改以下脚本中 **`<local_ip>`** 为 **Master 节点**私网 IP，并在 **Master 节点** 中执行。
...
1. 在 **Master 节点** 中，下载 calico 配置文件。
...
1. 在 **Node 节点** 中，执行上面保存的 **kubeadm join** 命令。
...
2. 在 **Master 节点** 中查看 Pod 状态，等待 **READY** 项全为 **1/1** 后，即为部署成功。
    ```shell
