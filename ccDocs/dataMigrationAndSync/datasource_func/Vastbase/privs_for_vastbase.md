---
id: privs_for_vastbase
title: Vastbase 需要的权限
description: CloudCanal 在做 Vastbase 源端或对端的数据迁移同步时，需要提供的账号有一些赋权。
---
本文介绍 Vastbase 作为源端或对端数据源迁移或同步数据时，CloudCanal 所需的账号权限。
...
- 如同步账号有高级权限，则系统会自动执行，否则请提前手动执行
  ...
  ```sql
  -- 以下为示例，请替换<>中的内容
  -- 创建 DDL 记录表 (cc_vb_ddl_capture_tab)
  CREATE TABLE IF NOT EXISTS public.cc_vb_ddl_capture_tab(
  ...
  -- 创建 DDL 捕获函数 (cc_vb_ddl_capture_func)
  CREATE OR REPLACE FUNCTION public.cc_vb_ddl_capture_func() RETURNS event_trigger
    LANGUAGE plpgsql
    SECURITY INVOKER
  AS $$
    declare ddl text;
    begin if (tg_tag in ('DROP INDEX','ALTER TABLE','DROP TABLE','CREATE INDEX','CREATE TABLE')) then
      ...
      insert into public.cc_vb_ddl_capture_tab(biz_ddl,biz_user,biz_txid,biz_tag,biz_db,biz_schema,biz_client_addr,biz_client_port,biz_event_time) values (ddl,current_user,cast(txid_current() as varchar(16)),tg_tag,current_database(),current_schema,inet_client_addr(),inet_client_port(),current_timestamp);
    end if;
  end;
  $$
  ...
  GRANT SELECT,INSERT,DELETE ON TABLE public.cc_vb_ddl_capture_tab TO public;
  GRANT SELECT,USAGE ON SEQUENCE public.cc_vb_ddl_capture_tab_id_seq TO public;
  GRANT EXECUTE ON FUNCTION public.cc_vb_ddl_capture_func() TO public;
          
  -- 创建 DDL 事件触发器 (cc_vb_ddl_capture_event)
  CREATE EVENT TRIGGER cc_vb_ddl_capture_event ON ddl_command_end EXECUTE PROCEDURE public.cc_vb_ddl_capture_func();
  ALTER EVENT TRIGGER cc_vb_ddl_capture_event ENABLE always;
  ```
