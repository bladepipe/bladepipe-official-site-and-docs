---
id:  set_kes_replica_identity
title: 设置 KingbaseES 源端表复制属性
...
KingbaseES 作为源端进行增量数据同步，需确保表的 replica identity 属性为 **FULL** ，但 **修改此属性会导致短暂锁表**，
...
创建任务过程中，如配置 **初始化表复制属性** 值为 **false**，创建的异步任务中会提示 **初始化 KingbaseES 增量复制位点** 失败，此时需要手动介入修改表属性。
