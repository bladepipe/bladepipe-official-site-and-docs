export const PolarDbX = {
    ...
    master_function: [
        {
            key: '设定分区键',
            desc: '支持表级别 DB(分库) 或 DB_TAB(分库分表) 设置，支持 HASH, WEEK, MM, DD, MMDD 分区函数'
        },
        {
            key: '按分区键写入',
            desc: '根据分区键设置，自动在 UPDATE 和 DELETE 操作加上分区键值，加速写入操作'
        },
        {
            key: '自定义代码',
    ...
    examples: [
        {
            key: 'MySQL 到 PolarDbX 数据迁移同步',
            desc: '文档：[MySQL 到 PolarDbX 数据迁移同步](https://www.clougence.com/blog/data_sync_sample/mysql_polardbx_sync)'
        }
    ],
    faq: [],
    mapping: [
