...
title: 使用钉钉审批
description: 本文档将介绍 CloudDM 的工单接入钉钉审批流。
---
...
:::info
您可以将 CloudDM 中 [单点登录(SSO)](../../integrations/sso/sso_dingtalk) 和工单整合进同一个钉钉应用。CloudDM 支持独立或分开配置。
:::
...
4. 点击 **权限管理**，给应用分配权限。
   ...
   OA审批
      ...
      工作流实例读权限
   ```
   ![common](../../assets/integrations/approval/dingtalk_approval/3.png)
5. 点击 **安全设置**，设置您部署 CloudDM 环境中公网出口 IP。
   ![common](../../assets/integrations/approval/dingtalk_approval/7.png)
...
1. 通过钉钉客户端或浏览器进入钉钉 [OA 审批管理后台](https://oa.dingtalk.com/)。
   ![common](../../assets/integrations/approval/dingtalk_approval/1.png)
2. 在 OA 审批管理后台点击 **创建新表单**，表单类型选择 **流程表单**。
   ![common](../../assets/integrations/approval/dingtalk_approval/2.png)
3. 在 **表单设计** 的步骤，按照情况添加必要的控件，在添加过程中请不要开启必填选项。表单内容请参考 **[配置钉钉表单](#config_form)**。
   ![common](../../assets/integrations/approval/dingtalk_approval/4.png)
4. 在 **流程设计** 的步骤，设置各节点的审批人及审批方式（<font color="red">需要注意：在设置节点审批人时 CloudDM 不支持发起人自选</font>）。
   ![common](../../assets/integrations/approval/dingtalk_approval/8.png)
...
- SQL 工单的表单按照如下内容填写。
   ...
   预计受影响行数（数字输入框）
   ```
   ![common](../../assets/integrations/approval/dingtalk_approval/ticket-sql.png)
   
   :::info
   注意：由于表单大小限制，表单如果内容超长会被截断，完整内容需到 CloudDM 控制台。
   - 单行输入框，400 长度
   - 多行输入框，4000 长度
   :::
...
- 权限工单的表单按照如下内容填写。
   ...
   申请的权限（明细/表格，填写方式请选择：表格）
      ...
      权限列表（多行输入框）
   ```
   ![common](../../assets/integrations/approval/dingtalk_approval/ticket-auth.png)
...
- 变更工单的表单按照如下内容填写。
   ...
   执行 SQL（多行输入框）
   ```
   ![common](../../assets/integrations/approval/dingtalk_approval/ticket-change.png)

## 配置消息通道 {#event_push}

1. 确保 CloudDM 已经开启钉钉审批（参考本文最开始部分）。
2. 回到钉钉应用开放平台，配置消息订阅通道。点击 **事件订阅** > **已完成接入，验证连接通道**，验证通过后点击 **保存**。
   ![common](../../assets/integrations/approval/dingtalk_approval/5.png)

### 订阅设置

1. 订阅设置。
   ![common](../../assets/integrations/approval/dingtalk_approval/6.png)
   ...
   :::
   ![common](../../assets/integrations/approval/dingtalk_approval/9.png)
2. 订阅地址填写：
   - 事件 **审批实例开始、结束**
...
3. 获取模版码：   
   ...
   ![common](../../assets/integrations/approval/dingtalk_approval/10.png)
