...
:::info
您可以将 CloudDM Team 中 [单点登录(SSO)](../../integrations/sso/sso_wechat) 和工单整合进同一个企业微信应用。CloudDM Team 支持独立或分开配置。
:::
...
**应用基本配置**
1. 点击 **应用管理** > **应用** > **创建应用**。
   ![common](../../assets/integrations/sso/sso_wechat/app-step1.png)
2. 填写应用的基础信息，涉及图标资源可以在 [资源下载](../../resource/resource_download) 中获取。应用可用范围选择组织机构的 **根部门(包含所有员工)**，然后点击 **创建应用**。
   ![common](../../assets/integrations/sso/sso_wechat/app-step2.png)
3. 在 **应用详情** 中可以获取，获取 **AgentId** 和 **Secret**。
   ![common](../../assets/integrations/sso/sso_wechat/app-step3.png)
4. 在 **我的企业** 页面底部可以获取 **企业 ID (CorpId)**。
   ![common](../../assets/integrations/sso/sso_wechat/app-step4.png)
5. 在 **应用详情** 中找到 **企业可信 IP** 卡片，点击 **设置**，填写您部署 CloudDM Team 环境中公网出口 IP。
   ![common](../../assets/integrations/sso/sso_wechat/app-step6.png)

## 表单配置参考 {#create_form}

1. 进入 [企业微信后台](https://work.weixin.qq.com/)，进入 **应用管理** > **审批应用**。
   ![common](../../assets/integrations/approval/wechat_approval/1.png)
2. **首次进入** 应用需要设置应用负责人，然后点击 **模版管理** > **添加模版**，在弹出框中选择 **自定义模版**。
   ![common](../../assets/integrations/approval/wechat_approval/2.png)
   ![common](../../assets/integrations/approval/wechat_approval/3.png)
2. 在 **表单设计** 的步骤，按照情况添加必要的控件，在添加过程中请不要开启必填选项。表单内容请参考 **[配置企业微信表单](#config_form)**。
   ![common](../../assets/integrations/approval/wechat_approval/4.png)
3. 在 **流程设计** 的步骤，设置各节点的审批人及审批方式（<font color="red">需要注意：在设置节点审批人时 CloudDM Team 不支持申请人自选</font>）。
   ![common](../../assets/integrations/approval/wechat_approval/5.png)
   ![common](../../assets/integrations/approval/wechat_approval/6.png)
4. 配置完成后，在页面最下方点击 **保存**。
   ![common](../../assets/integrations/approval/wechat_approval/7.png)
...
- SQL 工单的表单按照如下内容填写。
   ...
   预计受影响行数（文本）
   ```
   ![common](../../assets/integrations/approval/wechat_approval/ticket-sql.png)
...
- 权限工单的表单按照如下内容填写。
   ...
   申请的权限（明细，打印格式请选择：合并成一行打印）
      ...
      权限列表（多行文本）
   ```
   ![common](../../assets/integrations/approval/wechat_approval/ticket-auth.png)
...
- 变更工单的表单按照如下内容填写。
   ...
   执行 SQL（多行文本）
   ```
   ![common](../../assets/integrations/approval/wechat_approval/ticket-change.png)

## 配置消息通道 {#event_push}

1. 启用消息消息通道。
   ![common](../../assets/integrations/approval/wechat_approval/app-step7.png)
   ![common](../../assets/integrations/approval/wechat_approval/app-step8.png)
2. 接收消息服务器在配置时，需要
...
4. 在 **审批** 应用中设置 **开启回调通知的模版** 和 **可调用接口的应用**。
   ![common](../../assets/integrations/approval/wechat_approval/8.png)
   ![common](../../assets/integrations/approval/wechat_approval/9.png)
   ![common](../../assets/integrations/approval/wechat_approval/10.png)
...
4. 获取 “模版 URL”：
   ![common](../../assets/integrations/approval/wechat_approval/11.png)
