...
:::info
您可以将 CloudDM Team 中 [单点登录(SSO)](../../integrations/sso/sso_feishu) 和工单整合进同一个飞书应用。CloudDM Team 支持独立或分开配置。
:::
...
**应用基本配置**
1. 点击 **应用开发** > **飞书应用** > **创建应用**。
   ![common](../../assets/integrations/sso/sso_feishu/app-step1.png)
2. 填写应用的基础信息，并点击 **保存**。涉及图标资源可以在 [资源下载](../../resource/resource_download) 中获取。
   ![common](../../assets/integrations/sso/sso_feishu/app-step2.png)
3. 在 **凭证与基础信息信息**，获取 **App ID** 和 **App Secret**。
   ![common](../../assets/integrations/sso/sso_feishu/app-step3.png)
4. 点击 **权限管理** > **开通权限** > **应用身份权限**：
   ...
   通讯录
      ...
      通过手机号或邮箱获取用户 ID
   ```
   ![common](../../assets/integrations/sso/sso_feishu/app-step4.png)
5. 点击 **安全设置**。<br/>在 **IP 白名单** 选项下配置您 CloudDM Team 环境中公网出口 IP。
   ![common](../../assets/integrations/approval/feishu_approval/3.png)
6. 点击 **版本管理与发布**，发布应用。应用可用范围选择 **所有员工**。
   ![common](../../assets/integrations/sso/sso_feishu/app-step6.png)

## 表单配置参考 {#create_form}

1. 进入 [飞书审批管理后台](https://www.feishu.cn/approval/admin/approvalList)，在 **审批管理** > **创建审批** 的弹出页中点击 **创建自定义审批**。
   ![common](../../assets/integrations/approval/feishu_approval/4.png)
   ![common](../../assets/integrations/approval/feishu_approval/5.png)
2. 在 **表单设计** 的步骤，按照情况添加必要的控件，在添加过程中请不要开启必填选项。表单内容请参考 **[配置飞书表单](#config_form)**
   ![common](../../assets/integrations/approval/feishu_approval/6.png)
3. 在 **流程设计** 的步骤，设置各节点的审批人及审批方式（<font color="red">需要注意：在设置节点审批人时 CloudDM Team 不支持递交人自选</font>）。
   ![common](../../assets/integrations/approval/feishu_approval/7.png)
4. 配置完成后，右上角点击 **发布**。
   ![common](../../assets/integrations/approval/feishu_approval/8.png)
...
- SQL 工单的表单按照如下内容填写。
   ...
   预计受影响行数（数字）
   ```
   ![common](../../assets/integrations/approval/feishu_approval/ticket-sql.png)
...
- 权限工单的表单按照如下内容填写。
   ...
   申请的权限（明细/表格，排列方式请选择：横向明细）
      ...
      权限列表（多行文本）
   ```
   ![common](../../assets/integrations/approval/feishu_approval/ticket-auth.png)
...
- 变更工单的表单按照如下内容填写。
   ...
   执行 SQL（多行文本）
   ```
   ![common](../../assets/integrations/approval/feishu_approval/ticket-change.png)

## 配置消息订阅 {#event_push}

1. 确保 CloudDM Team 已经开启钉钉审批（参考本文最开始部分）。
2. 回到飞书开发者后台，配置消息订阅通道。点击 **事件与回调**，在 **事件配置** > **订阅方式** 中选择 **使用长连接接收事件** 点击 **保存**。
   ![common](../../assets/integrations/approval/feishu_approval/9.png)
3. 点击 **添加事件**。添加 **审批实例状态变更** 和 **审批任务状态变更** 事件。
   ![common](../../assets/integrations/approval/feishu_approval/10.png)
...
4. 获取 “模版 URL”：
   ![common](../../assets/integrations/approval/feishu_approval/11.png)
