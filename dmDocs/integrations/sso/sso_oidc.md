...
description: 本文档主要介绍如何将 CloudDM 产品接入 OpenID Connect (OIDC) 以实现统一身份认证。
---

![w600_h200](../../assets/integrations/sso/sso_oidc/use-oidc-cn.png)

CloudDM 允许您的组织使用企业自身 **OpenID Connect (OIDC) 服务** 以实现统一身份认证。
...
- 当启用后 CloudDM 的账号有效性验证将会由 **OIDC** 验证。
...
- 登录完成后 OP 会将浏览器跳转回 CloudDM 并携带 Authorization code 代码。
- CloudDM 根据 Authorization code 代码向 OP 获取用户信息以完成登录动作。
  
## 如何配置

CloudDM 开启 OIDC 认证步骤如下：
...
- **oidcLoginWellKnownUrl** 参数：根据 OpenID Connect (OIDC) 协议标准，每一个 OP 都会提供 [Well-KnownUrl](https://openid.net/specs/openid-connect-discovery-1_0.html#WellKnownRegistry)，
   除机要信息外所有配置信息均通过 Well-KnownUrl 获取。
...
- **oidcLoginScope** 参数：请参考 [详细信息](https://openid.net/specs/openid-connect-core-1_0.html#ScopeClaims)。
- **oidcLoginRoleMap** 参数
...
- 首次进入控制台时会根据其 oidcLoginRoleMap 参数配置分配 CloudDM 用户角色。
:::
...
4. 请确保您的 Client 具备以下配置：
  - `Valid redirect URIs` 选项请填写 CloudDM 的首页地址（如：`http://192.168.0.100:8222/*` ）
  - `Web origins` 选项同上
...
:::warning
在 CloudDM 的实现机制中由于不会维护 OIDC 的 id_token 活跃性，因此为了系统正常退出需要将以下两个属性参数保持一致。
