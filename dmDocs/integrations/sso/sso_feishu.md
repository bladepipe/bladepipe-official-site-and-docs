...
id: sso_feishu
title: 飞书身份认证
description: 本文档主要介绍如何将 CloudDM 产品接入飞书以实现统一身份认证。
---

![w400_h280](../../assets/integrations/sso/sso_feishu/use-feishu-cn.png)

CloudDM 允许您的组织在使用 [飞书](https://www.feishu.cn/) 时，通过团队为您分配的飞书账号进行登录，从而实现统一身份认证。
...
- 当启用后 CloudDM 的账号有效性验证将会由 **飞书** 验证。
...
- 登录完成后飞书会将浏览器跳转回 CloudDM 并携带 Authorization code 代码。
- CloudDM 根据 Authorization code 代码向飞书获取用户信息以完成登录动作。
...
- 首次进入控制台时会根据其 feishuLoginRoleMap 参数配置分配 CloudDM 用户角色。
:::
...
:::info
您可以将 [飞书审批](../approval/feishu_approval.md) 和登录整合进同一个应用，CloudDM 支持独立或分开配置它们。
:::
...
3. 点击 **安全设置**。<br/>在 **IP 白名单** 选项下配置您 CloudDM 环境中公网出口 IP。<br/>在 **重定向 URL** 中设置您部署 CloudDM 回调地址（例：`http://192.168.0.100:8222/callback/auth`）。
   ![common](../../assets/integrations/sso/sso_feishu/app-step5.png)
