---
id: extension_cloudcanal
title: 集成 CloudCanal
description: 本文主要介绍如何在已部署使用 CloudDM Team 后，添加 CloudCanal 产品并在同一个控制台中使用它们。
---
...
**[CloudCanal](https://www.clougence.com/cc-doc/intro/product_intro)** 是 CloudDM Team 同一个厂商推出的在线数据实时数据同步产品，
...
![common w650_h300](../../assets/maintain/extension/extension_cloudcanal/architecture.png)

## 整合方式

目前支持采用全新部署方式整合两款产品，具体是指如下场景：

1. 使用已有 CloudCanal 产品整合全新 CloudDM Team 产品。
...
3. 已经在使用中的 CloudCanal、CloudDM Team 产品整合到一起。
   - 不支持
...
:::info
如果 CloudCanal、CloudDM Team 均为 Docker 部署，且位于同一个 Docker 服务中。<br/>
...
2. 检查 console.properties 配置文件。
    ...
    ```
3. 修改 CloudCanal 配置，确保下面配置项和 CloudDM Team 配置内容相同。
    ```test
...
1. 访问 CloudDM Team、CloudCanal 任意一款产品的服务地址。
   - CloudDM Team 通常地址为：_**http://&lt;部署服务器IP&gt;:8222**_
...
3. 添加产品集群 **系统设置** > **产品集群管理** > **添加产品集群**(按钮)。
   - 按照如下表格添加两个产品：
...
- **产品集群名称**
  - 一个具有辨识度的名字。
- **版本号**
  - 在登录 CloudDM Team、CloudCanal 产品后右上角会有对应版本号。
...
1. CloudDM Team、CloudCanal 两款产品登录后 Logo 自动变成 **ClouGence RDP**。
2. 从 CloudDM Team 控制台进入，可访问两个产品所有能力。
