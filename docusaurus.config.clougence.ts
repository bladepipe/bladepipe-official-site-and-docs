...
const config: Config = {
  ...
  customFields: {
    ...
    announcement: {
      ...
      text: '🎉 CloudCanal 上线 V6.0.0.0：支持 Hologres、CloudBerry、Snowflake 数据源；产品运行环境升级至 JDK17', // 公告文本（支持 i18n）
      linkUrl: '/docs/releaseNote/rn-cloudcanal-6-0-0-0', // 链接地址（整个区域可点击）
      endDate: '2026-5-30T23:59:59' // 可选：结束日期（ISO 格式），如 '2025-12-31T23:59:59'，超过此时间后不再显示。不设置则永久显示（当 enabled 为 true 时）
    }
  },
